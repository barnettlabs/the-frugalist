	...
	fetcher: async (): Promise<DashboardStats> => {
		...
		try {
			...
			try {
				...
				return {
					financeCount: Array.isArray(financeRes.data) ? financeRes.data.length : 0,
					leaseCount: Array.isArray(leaseRes.data) ? leaseRes.data.length : 0,
					watchCount: Array.isArray(trackerRes.data?.tracked_products) ? trackerRes.data.tracked_products.length : 0,
				};
			} catch {
