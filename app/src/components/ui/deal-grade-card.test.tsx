...
describe('DealGradeCard', () => {
	...
	it('renders the initial CTA', () => {
		setup(
			withClient(<DealGradeCard agentSlug="deal-grade-finance" calculatorType="finance" inputs={{ msrp: 35000 }} />)
		);
	...
	it('shows the grade after a successful run', async () => {
		...
		expect(mockPost.mock.calls[1][0]).toBe('/ai/agents/deal-grade-finance/run');
	});
	...
	it('surfaces a 429 rate-limit message', async () => {
		...
		const { user } = setup(
			withClient(<DealGradeCard agentSlug="deal-grade-finance" calculatorType="finance" inputs={{}} />)
		);
