  ...
  const { mutate, isPending } = useGradeDeal({
    ...
    onError: (error) => {
      ...
      setErrorMessage(
        apiMessage ??
          (status === 429
            ? 'Daily limit reached for this agent.'
    ...
    <View className="rounded-lg border border-border bg-surface p-4 my-3">
      ...
      {!result && !errorMessage && !isPending && (
        <Text className="text-sm text-text-muted mt-1">
          Get an AI take on whether this deal is strong, average, or one to walk
          away from.
        </Text>
      )}
      ...
      {errorMessage && !isPending && (
        <Text className="text-sm text-danger bg-danger/10 rounded p-2 mt-3">
          {errorMessage}
        </Text>
      )}
      ...
      {result && !isPending && (
        <View className="mt-3">
          <View className="flex-row items-start gap-3">
            <View
              className={`w-14 h-14 rounded-md border items-center justify-center ${gradeStyle(result.grade)}`}
            >
              <Text className="text-2xl font-semibold">{result.grade}</Text>
            </View>
            <View className="flex-1">
              <Text className="text-sm text-primary capitalize">
                {result.rating.replaceAll('_', ' ')}
              </Text>
              <Text className="text-sm text-text-muted mt-1">
                {result.summary}
              </Text>
          ...
          {cached && (
            <Text className="text-[10px] text-text-muted mt-2">
              Cached response — identical inputs.
            </Text>
          )}
        </View>
      )}
