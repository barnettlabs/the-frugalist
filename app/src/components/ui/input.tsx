...
const inputTv = tv({
	...
	variants: {
		...
		disabled: {
			true: {
				input: 'bg-surface-dark-light text-text-muted-light dark:bg-surface-dark-dark dark:text-text-muted-dark',
			},
		},
	},
	defaultVariants: {
...
export const Input = React.forwardRef<NTextInput, NInputProps>((props, ref) => {
	...
	return (
		<View className={styles.container()}>
			...
			/>
			{helperText && !error && (
				<Text className="mt-1.5 text-xs text-text-muted-light dark:text-text-muted-dark">{helperText}</Text>
			)}
			{error && (
				<Text testID={testID ? `${testID}-error` : undefined} className="mt-1.5 text-xs text-danger">
					{error}
				</Text>
			)}
		</View>
