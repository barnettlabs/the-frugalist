...
export class FinanceCalculator {
	...
	calculateMonthlyPrincipalAmount(monthlyInterestAmount: number, totalPaidForMonth: number): number {
	...
	getExtraPaymentForMonth(month: number, extraPayments: ExtraPayment[]): number {
		let totalExtra = 0;
	...
	getChartData() {
		...
		return {
			...
			principal: schedule.map(payment => payment.principalPayment - payment.extraPayment),
