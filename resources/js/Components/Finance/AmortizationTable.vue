        ...
        <div v-if="amortization && amortization.schedule.length > 0">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-blue-50 p-4 rounded-lg">
                    ...
                    <div class="text-lg font-bold text-blue-900">${{ formatCurrency(amortization.totalInterest) }}</div>
                </div>
                <div class="bg-green-50 p-4 rounded-lg">
                    <div class="text-sm text-green-600 font-medium">Total Principal</div>
                    <div class="text-lg font-bold text-green-900">${{ formatCurrency(amortization.totalPrincipal) }}</div>
                </div>
                ...
                <table class="min-w-full table-auto">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th v-if="showWithExtra" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Extra</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Principal</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="payment in displayedPayments" :key="payment.month" class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-sm text-gray-900">${{ formatCurrency(payment.payment) }}</td>
                            <td v-if="showWithExtra" class="px-3 py-2 text-sm text-green-600">
                                ${{ formatCurrency(payment.extraPayment) }}
                            </td>
                            <td class="px-3 py-2 text-sm text-blue-600">${{ formatCurrency(payment.principalPayment) }}</td>
                            <td class="px-3 py-2 text-sm text-red-600">${{ formatCurrency(payment.interestPayment) }}</td>
                            <td class="px-3 py-2 text-sm text-gray-900">${{ formatCurrency(payment.remainingBalance) }}</td>
                        </tr>
                    </tbody>
                </table>
