...
import { ArrowPathIcon, ExclamationTriangleIcon, LightBulbIcon, SparklesIcon } from '@heroicons/vue/24/outline';
import { ref } from 'vue';
	...
	<section class="card p-5">
		...
		<div v-if="result" class="space-y-4">
			<div class="flex items-start gap-4">
				<div
					:class="[
						'inline-flex items-center justify-center w-14 h-14 rounded-lg border font-display text-2xl tracking-tight',
			...
			<div v-if="result.red_flags?.length">
				<p class="eyebrow flex items-center gap-1.5 mb-1"><ExclamationTriangleIcon class="h-3.5 w-3.5" /> Watch out</p>
			...
			<div v-if="result.tips?.length">
				<p class="eyebrow flex items-center gap-1.5 mb-1"><LightBulbIcon class="h-3.5 w-3.5" /> Tips</p>
				<ul class="text-sm text-text-muted list-disc pl-5 space-y-0.5">
		...
		<div class="mt-4 flex items-center gap-2">
			...
			<button v-if="result" class="text-sm text-text-muted hover:text-primary transition-colors" @click="reset">
				Clear
			</button>
		</div>
	</section>
