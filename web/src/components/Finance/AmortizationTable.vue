    ...
    <div v-if="amortization && amortization?.schedule?.length > 0" class="p-5 sm:p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-border border border-border rounded-md overflow-hidden">
        ...
        <div class="bg-surface p-4">
          ...
          <p class="figure text-xl leading-none" :class="amortization.monthsSaved > 0 ? 'text-success' : 'text-text-muted'">
        ...
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-border">
            <tr v-for="payment in displayedPayments" :key="payment.month" class="hover:bg-tan/30 transition-colors">
              ...
              <td class="px-3 py-2.5 text-sm numeral text-primary text-right">${{ formatCurrency(payment.payment) }}</td>
              <td v-if="showWithExtra" class="px-3 py-2.5 text-sm numeral text-success text-right">
                ${{ formatCurrency(payment.extraPayment) }}
              </td>
              <td class="px-3 py-2.5 text-sm numeral text-accent-dark text-right">${{ formatCurrency(payment.principalPayment) }}</td>
              <td class="px-3 py-2.5 text-sm numeral text-warning text-right">${{ formatCurrency(payment.interestPayment) }}</td>
              <td class="px-3 py-2.5 text-sm numeral text-primary text-right">${{ formatCurrency(payment.remainingBalance) }}</td>
            </tr>
          </tbody>
        </table>
