...
<template>
  <section class="card overflow-hidden">
        ...
        <div>
          ...
          <dl class="divide-y divide-border border-y border-border">
            <div class="flex justify-between py-2.5">
              ...
              <dd class="text-sm numeral text-primary">$0 <span class="text-text-muted">(subject to wear/mileage)</span></dd>
            </div>
            <div class="flex justify-between py-2.5">
            ...
            <table class="min-w-full text-sm">
              ...
              <tbody class="divide-y divide-border">
                ...
                >
                  ...
                  <td class="py-2.5 px-2 numeral text-primary text-right">${{ formatCurrency(scenario.buyoutPrice) }}</td>
                  <td class="py-2.5 px-2 numeral text-primary text-right">${{ formatCurrency(scenario.estimatedMarketValue) }}</td>
                  <td class="py-2.5 px-2 numeral text-right" :class="scenario.equity > 0 ? 'text-success' : 'text-danger'">
                    ${{ formatCurrency(scenario.equity) }}
                  </td>
                  <td class="py-2.5 px-2 numeral text-primary text-right">${{ formatCurrency(scenario.totalPaidSoFar) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          <p class="mt-3 text-xs text-text-muted/80 italic">
            Early buyout prices are estimates and vary by leasing company. Market values use linear depreciation, which is approximate.
          </p>
        </div>
    ...
    <div v-else class="p-10 text-center text-sm text-text-muted">
      Enter lease details to see buyout analysis.
    </div>
  </section>
</template>
