...
import { PencilSquareIcon, BeakerIcon, ClockIcon, ArrowUturnLeftIcon } from '@heroicons/vue/24/outline'
import Spinner from '@/components/Spinner.vue'
import Modal from '@/components/Modal.vue'
import {
  adminAiAgentsApi, adminAiProvidersApi,
  type AiAgent, type AiProvider, type AiAgentVersion,
} from '@/api/admin-ai'
        ...
        <tbody>
          <tr v-for="a in agents" :key="a.id" class="border-t border-border">
            ...
            <td class="px-4 py-3 text-right">
              <button @click="openEdit(a)" class="p-2 text-text-muted hover:text-primary transition-colors" title="Edit">
      ...
      <div v-if="editing" class="p-6 max-h-[90vh] overflow-y-auto">
        <header class="flex items-start justify-between mb-6">
          ...
          <div class="flex items-center gap-2">
            <button type="button" @click="editing = null" class="px-4 py-2 text-sm rounded-md border border-border hover:bg-surface-dark transition-colors">
              Close
            </button>
            <button type="button" @click="save" :disabled="saving" class="px-4 py-2 text-sm rounded-md bg-primary text-surface hover:bg-primary-light transition-colors disabled:opacity-50">
              {{ saving ? 'Saving…' : 'Save' }}
            </button>
          </div>
        </header>
        ...
        <div class="grid grid-cols-12 gap-6">
          <section class="col-span-12 lg:col-span-7 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="editing.name" type="text" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none" />
              </div>
              <div>
                <label class="block text-sm font-medium text-text-muted mb-1">Provider</label>
                <select v-model="editing.provider_id" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none">
              ...
              <div>
                ...
                <input v-model="editing.model" type="text" placeholder="leave blank to use provider default" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none numeral" />
              </div>
              <div>
                <label class="block text-sm font-medium text-text-muted mb-1">Response format</label>
                <select v-model="editing.response_format" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none">
            ...
            <div>
              ...
              <textarea v-model="editing.system_prompt" rows="4" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none font-mono"></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-text-muted mb-1">User prompt template</label>
              <p class="text-xs text-text-muted mb-1">
                Placeholders: <code>{{ '{{inputs_json}}' }}</code>, <code>{{ '{{computed_json}}' }}</code>, <code>{{ '{{context_json}}' }}</code>, or dotted paths like <code>{{ '{{inputs.msrp}}' }}</code>.
              </p>
              <textarea v-model="editing.user_prompt_template" rows="10" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none font-mono"></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-text-muted mb-1">Output JSON schema (optional)</label>
              <textarea :value="outputSchemaText" @input="outputSchemaText = ($event.target as HTMLTextAreaElement).value" rows="6" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none font-mono"></textarea>
            ...
            <div class="grid grid-cols-3 gap-4">
              <div>
                ...
                <input v-model.number="editing.temperature" type="number" min="0" max="2" step="0.05" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none numeral" />
              </div>
              <div>
                <label class="block text-sm font-medium text-text-muted mb-1">Top P</label>
                <input v-model.number="editing.top_p" type="number" min="0" max="1" step="0.05" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none numeral" />
              </div>
              <div>
                <label class="block text-sm font-medium text-text-muted mb-1">Max tokens</label>
                <input v-model.number="editing.max_tokens" type="number" min="1" max="8192" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none numeral" />
              </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-4">
              ...
              <div class="flex items-center gap-2 text-sm">
                ...
                <input v-model.number="editing.rate_limit_per_user_day" type="number" min="0" class="w-24 rounded-md border border-border px-2 py-1 text-sm focus:border-accent focus:outline-none numeral" />
              </div>
            </div>
          </section>
          ...
          <aside class="col-span-12 lg:col-span-5 space-y-6">
            <section>
              ...
              <textarea v-model="previewSampleJson" rows="8" class="w-full rounded-md border border-border px-3 py-2 text-xs focus:border-accent focus:outline-none font-mono"></textarea>
              <button @click="runPreview" :disabled="previewLoading" class="mt-2 px-3 py-1.5 text-sm rounded-md bg-accent/10 text-accent hover:bg-accent/20 transition-colors disabled:opacity-50">
                {{ previewLoading ? 'Running…' : 'Run preview' }}
              </button>
              <pre v-if="previewResult" class="mt-3 p-3 rounded-md bg-surface-dark/50 text-xs overflow-x-auto numeral max-h-72">{{ JSON.stringify(previewResult, null, 2) }}</pre>
            </section>
            ...
            <section>
              ...
              <ul class="space-y-1 max-h-72 overflow-y-auto">
                <li v-for="v in versions" :key="v.id" class="flex items-center justify-between text-xs p-2 rounded hover:bg-surface-dark/30">
                  <span class="numeral">v{{ v.version }} · {{ new Date(v.created_at).toLocaleString() }}</span>
                  <button @click="rollback(v)" class="text-text-muted hover:text-accent inline-flex items-center gap-1" title="Roll back">
                    <ArrowUturnLeftIcon class="h-3.5 w-3.5" /> Roll back
                  </button>
                </li>
                <li v-if="!versions.length" class="text-xs text-text-muted">No version history yet — saving will start the log.</li>
              </ul>
            </section>
          </aside>
        </div>
