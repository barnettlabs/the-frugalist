...
import {
  adminAiInvocationsApi, adminAiAgentsApi,
  type AiInvocation, type AiAgent,
} from '@/api/admin-ai'
...
watch([filterAgent, filterErrors], () => { page.value = 1; fetch() })
watch(page, fetch)
...
<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-4 text-sm">
      <select v-model="filterAgent" class="rounded-md border border-border px-3 py-2 focus:border-accent focus:outline-none">
    ...
    <div v-else class="card overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="i in invocations" :key="i.id" @click="showing = i" class="border-t border-border cursor-pointer hover:bg-surface-dark/30">
            <td class="px-4 py-3 text-xs text-text-muted numeral">{{ new Date(i.created_at).toLocaleString() }}</td>
            <td class="px-4 py-3 text-xs">{{ i.agent?.slug || '—' }} <span class="text-text-muted">v{{ i.agent_version }}</span></td>
            <td class="px-4 py-3">
    ...
    <div class="flex items-center justify-end gap-2 mt-3 text-sm">
      <button :disabled="page <= 1" @click="page--" class="px-3 py-1.5 rounded border border-border disabled:opacity-40">Prev</button>
      <span class="numeral text-text-muted">{{ page }} / {{ lastPage }}</span>
      <button :disabled="page >= lastPage" @click="page++" class="px-3 py-1.5 rounded border border-border disabled:opacity-40">Next</button>
    </div>
      ...
      <div v-if="showing" class="p-6 max-h-[90vh] overflow-y-auto">
        <header class="flex items-center justify-between mb-4">
          <div>
            ...
            <h2 class="font-display text-2xl text-primary tracking-tight mt-1">{{ showing.agent?.name }} <span class="text-text-muted text-base">v{{ showing.agent_version }}</span></h2>
          </div>
          <button @click="showing = null" class="px-4 py-2 text-sm rounded-md border border-border hover:bg-surface-dark transition-colors">Close</button>
        </header>

        <dl class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs mb-6">
          <div><dt class="eyebrow">Status</dt><dd><span :class="['px-2 py-0.5 rounded text-xs', statusColor(showing.status)]">{{ showing.status }}</span></dd></div>
          <div><dt class="eyebrow">Latency</dt><dd class="numeral">{{ fmtMs(showing.latency_ms) }}</dd></div>
          <div><dt class="eyebrow">Prompt tokens</dt><dd class="numeral">{{ showing.prompt_tokens || 0 }}</dd></div>
          <div><dt class="eyebrow">Completion tokens</dt><dd class="numeral">{{ showing.completion_tokens || 0 }}</dd></div>
          <div><dt class="eyebrow">Provider</dt><dd>{{ showing.provider?.name }}</dd></div>
          <div><dt class="eyebrow">Model</dt><dd class="numeral">{{ showing.model }}</dd></div>
          <div><dt class="eyebrow">Context key</dt><dd class="numeral">{{ showing.context_key || '—' }}</dd></div>
          <div><dt class="eyebrow">Cached</dt><dd>{{ showing.cached ? 'yes' : 'no' }}</dd></div>
        </dl>
        ...
        <section class="mb-6">
          ...
          <pre class="p-3 rounded bg-surface-dark/50 text-xs overflow-x-auto numeral">{{ JSON.stringify(showing.request_payload, null, 2) }}</pre>
        </section>
        <section class="mb-6">
          <h3 class="eyebrow mb-2">Parsed response</h3>
          <pre class="p-3 rounded bg-surface-dark/50 text-xs overflow-x-auto numeral">{{ JSON.stringify(showing.response, null, 2) }}</pre>
        </section>
