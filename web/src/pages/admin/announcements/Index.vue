...
import { adminAnnouncementsApi, type AdminAnnouncement } from '@/api/admin'
      ...
      <article v-for="a in announcements" :key="a.id" class="card p-5">
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex items-center gap-1 flex-shrink-0">
            <button @click="openEdit(a)" class="p-2 text-text-muted hover:text-primary transition-colors">
              <PencilSquareIcon class="h-4 w-4" />
            </button>
            <button @click="deleting = a" class="p-2 text-text-muted hover:text-danger transition-colors">
      ...
      <form @submit.prevent="save" class="p-6">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input v-model="form.title" type="text" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none" />
          ...
          <div>
            ...
            <textarea v-model="form.message" rows="6" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:border-accent focus:outline-none"></textarea>
