...
<template>
  <div class="max-w-3xl mx-auto">
    <RouterLink to="/admin/bug-reports" class="inline-flex items-center gap-1 text-sm text-text-muted hover:text-primary mb-6 transition-colors">
    ...
    <div v-else-if="report">
      <div class="flex items-start justify-between gap-4 mb-6">
        ...
        <button @click="showDelete = true" class="p-2 text-text-muted hover:text-danger transition-colors" title="Delete">
      ...
      <div class="card p-5 mb-4">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <button v-for="s in STATUSES" :key="s" @click="updateStatus(s)" :disabled="saving"
            class="px-3 py-1.5 rounded-md text-xs font-medium transition-colors"
            :class="report.status === s
      ...
      <div v-if="report.metadata" class="card p-5">
        ...
        <pre class="text-xs numeral overflow-x-auto bg-surface-dark/50 p-3 rounded-md">{{ JSON.stringify(report.metadata, null, 2) }}</pre>
      </div>
    </div>
