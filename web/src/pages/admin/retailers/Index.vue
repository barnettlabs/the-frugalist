...
import { CheckCircleIcon, PencilSquareIcon, PlusIcon, TrashIcon, XCircleIcon } from '@heroicons/vue/24/outline';
import { onMounted, reactive, ref } from 'vue';

import { type AdminRetailer, adminRetailersApi } from '@/api/admin';
...
const fetchRetailers = async () => {
	loading.value = true;
	try {
		const [list, slugs] = await Promise.all([adminRetailersApi.list(), adminRetailersApi.availableSlugs()]);
	...
	<div class="max-w-6xl mx-auto">
		<header class="flex items-end justify-between mb-8">
			<div>
				...
				<p class="text-sm text-text-muted mt-2">Toggle integrations, manage API keys, and configure rate limits.</p>
			</div>
			<button
