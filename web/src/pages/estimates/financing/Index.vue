  ...
  <main class="py-12 flex-1">
    <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <PageHeader
        ...
        description="Calculate monthly payments, total interest, and compare financing options"
        back-link="/dashboard"
          ...
          <section aria-labelledby="finance-estimates-overview-title">
            <ul class="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div v-else-if="!vehicleFinanceSheets.length" class="col-span-full">
                <div
                  class="bg-surface/80 backdrop-blur-sm rounded-lg border border-border p-12 text-center"
                >
                  <div
                    class="p-4 rounded-xl bg-info/10 w-16 h-16 mx-auto mb-4 flex items-center justify-center"
                  >
                  ...
                  <RouterLink to="/estimates/financing/create">
                    <button
                      class="bg-accent hover:bg-accent-dark text-white px-6 py-3 rounded-lg font-medium transition-all duration-150 flex items-center space-x-2 mx-auto"
                    >
                      <PlusIcon class="h-5 w-5" />
              ...
              <template v-else>
                ...
                >
                  <div
                    class="bg-surface/80 backdrop-blur-sm rounded-lg border border-border p-4 hover:border-accent/30 transition-all group"
                  >
                    <!-- Header -->
                    <div class="flex items-start space-x-3 mb-3">
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2 mb-1">
                          <h3
                            class="text-base font-medium text-primary group-hover:text-accent transition-colors truncate"
                          >
                    ...
                    <div class="bg-background rounded-lg p-3 mb-3">
                      <div class="text-center">
                        <span class="text-text-muted text-xs uppercase tracking-wide block mb-1"
                    ...
                    <div class="grid grid-cols-3 gap-2">
                      <RouterLink :to="`/estimates/financing/${sheet.id}/edit`">
                        <button
                          class="w-full bg-accent hover:bg-accent-dark text-white px-2 py-2 rounded-lg text-xs font-medium transition-all duration-150 flex items-center justify-center space-x-1"
                        >
                    ...
                    <div v-if="isCardExpanded(sheet.id)" class="space-y-4 mt-4">
                      <div>
                        <h4 class="font-medium text-primary text-sm mb-2">Vehicle Information</h4>
                        <div class="text-sm text-text-muted space-y-1">
                          <div class="flex justify-between">
                            <span>MSRP:</span>
                            <span class="font-medium text-primary">${{ formatCurrency(sheet.msrp || 0) }}</span>
                          </div>
                          <div class="flex justify-between">
                            <span>Purchase Price:</span>
                            <span class="font-medium text-primary">${{ formatCurrency(getSheetCalculations(sheet).purchasePrice) }}</span>
                          </div>
                        </div>
                      </div>

                      <div class="bg-background rounded-lg p-3">
                        <h4 class="font-medium text-primary text-sm mb-2">Financing Terms</h4>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                          <div>
                            <span class="text-text-muted text-xs uppercase tracking-wide block">Interest Rate</span>
                            <div class="font-medium text-primary">{{ sheet.interest_rate || 0 }}%</div>
                          </div>
                          <div>
                            <span class="text-text-muted text-xs uppercase tracking-wide block">Loan Term</span>
                            <div class="font-medium text-primary">{{ sheet.finance_term || 0 }} mo</div>
                          </div>
                          <div>
                ...
                <li class="list-none">
                  <RouterLink to="/estimates/financing/create">
                    <div
                      class="bg-surface/60 backdrop-blur-sm rounded-lg p-8 text-center border-2 border-dashed border-accent/20 hover:border-accent/40 transition-colors cursor-pointer group"
                    >
                      ...
                      <p class="text-text-muted text-sm">Start a new vehicle financing calculation</p>
                    </div>
                  </RouterLink>
                </li>
              </template>
          ...
          <div class="bg-surface/80 backdrop-blur-sm rounded-lg border border-border p-6">
            ...
            <div class="space-y-4">
              <div class="flex justify-between items-center">
                ...
                <span class="font-medium text-accent">{{ vehicleFinanceSheets.length }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-sm text-text-muted">Recent Activity</span>
                <span class="text-xs text-text-muted">{{ vehicleFinanceSheets.length ? 'Active' : 'No activity' }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-surface/80 backdrop-blur-sm rounded-lg border border-border p-6">
            ...
            <div class="space-y-3">
              <div class="flex items-start space-x-3">
                ...
                <p class="text-sm text-text-muted">Negotiate the total price before discussing financing terms</p>
              </div>
              <div class="flex items-start space-x-3">
                <div class="w-2 h-2 bg-accent rounded-full mt-2 flex-shrink-0"></div>
                <p class="text-sm text-text-muted">Compare multiple financing scenarios to see total cost differences</p>
              </div>
              <div class="flex items-start space-x-3">
                <div class="w-2 h-2 bg-accent rounded-full mt-2 flex-shrink-0"></div>
                <p class="text-sm text-text-muted">Know your credit score and get pre-approved before shopping</p>
              </div>
            </div>
          </div>
        </div>
