        ...
        <article class="grid grid-cols-12 gap-8 lg:gap-12">
          ...
          <div class="col-span-12 lg:col-span-9 space-y-14">
            ...
            <section v-if="content.keywords && content.keywords.length > 0" id="concepts">
              ...
              <dl class="grid grid-cols-1 md:grid-cols-2 gap-px bg-border border border-border rounded-md overflow-hidden">
                <div
                ...
                >
                  ...
                  <dd v-if="kw.description" class="mt-1 text-sm text-text-muted leading-relaxed">{{ kw.description }}</dd>
                </div>
              </dl>
            </section>
              ...
              <dl class="divide-y divide-border border-y border-border">
                ...
                >
                  <dt class="col-span-12 sm:col-span-3 font-display text-lg text-primary tracking-tight">
                    <span class="numeral text-xs text-text-muted block mb-1">{{ String(index + 1).padStart(2, '0') }}</span>
                    {{ term.term }}
                  </dt>
                  <dd class="col-span-12 sm:col-span-9 text-sm text-primary leading-relaxed">
                    {{ term.definition }}
                    <p v-if="term.example" class="mt-3 italic font-display text-base text-text-muted border-l-2 border-accent pl-4">
            ...
            <section v-if="content.tips && content.tips.length > 0" id="tips">
              ...
              <ol class="space-y-4">
                ...
                >
                  <span class="col-span-2 sm:col-span-1 numeral text-xs text-text-muted">№ {{ String(index + 1).padStart(2, '0') }}</span>
