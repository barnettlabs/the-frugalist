...
import { type TrackedProduct, watchApi } from '@/api/watch';
	...
	<div class="pb-16">
		...
		<main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-2">
			...
			<div
				...
				class="flex items-center gap-1 mb-8 border border-border rounded-md p-1 bg-surface w-fit"
			>
				<button
			...
			<div
				...
				class="bg-surface border border-border rounded-md p-10 text-center"
			>
				<p class="text-text-muted text-sm">
					No {{ activeFilter === 'active' ? 'active' : 'paused' }} products.
					<button class="text-accent-dark hover:underline" @click="activeFilter = 'all'">View all</button>
				</p>
			</div>
					...
					<article class="card h-full flex flex-col overflow-hidden">
						...
						<div class="flex items-center justify-between px-4 pt-3 pb-2 border-b border-border">
							<div class="flex items-center gap-2 min-w-0">
								...
								<span class="inline-flex items-center gap-0.5 text-text-muted ml-1">
									...
									<ArchiveBoxIcon
										v-if="product.watch_type === 'stock' || product.watch_type === 'both'"
										class="h-3 w-3"
									/>
								</span>
							</div>
						...
						<div class="flex gap-4 p-4">
							...
							>
								...
								<span
									v-if="getDistanceToTarget(product)?.reached"
							...
							<div class="flex-1 min-w-0 flex flex-col">
								<h3
									class="font-display text-[1.0625rem] text-primary tracking-tight leading-snug line-clamp-2 group-hover:text-accent-dark transition-colors"
								>
								...
								<div class="mt-auto pt-3 flex items-baseline gap-2">
									...
									<span
										v-if="product.retail_price && product.current_price && product.current_price < product.retail_price"
